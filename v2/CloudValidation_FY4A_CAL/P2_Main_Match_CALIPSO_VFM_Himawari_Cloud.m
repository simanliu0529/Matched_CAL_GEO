...
Reg.LatMin = -60; %纬度
Reg.LatMax = 60;
starttime = datenum('20230601', 'yyyymmdd'); % 时间范围在同一年内
endtime = datenum('20230701', 'yyyymmdd');
startvec = datevec(starttime);
cur_month = startvec(:,2);
cur_year = startvec(:,1);
%% 读取数据——根据自己的数据存储位置修改路径
dirH8 = 'J:\H8Data\'; %加载葵花数据
Outpath = 'J:\H8Data\Matched_P2\';
% fileList_H8 = Fun_filesTraversal([dirH8,'CLM1\'],'*_CloudMask.mat'); % 以读取H8的云掩膜
% fileNum_H8 = size(fileList_H8,1);
for time = starttime : 1 : endtime %每日一个文件
% for fileH8_i = 1:fileNum_H8
    ...
    year = timevec(:,1);
    month = timevec(:,2);
    %如果跨月，则保存CLP，保存后清空CLP。
    if month ~= cur_month
        if ~exist(Outpath,'dir')
            mkdir(Outpath);
        end
        save([Outpath,'Mated_CloudPropertyTable_',[num2str(cur_year),num2str(cur_month,'%02d')],'.mat'],'Mated_CloudTable','-v7.3');
        clear Mated_CloudMaskTable
        cur_month = month; cur_year = year;
    end
    H8_Year = num2str(timevec(1));
    H8_Month = num2str(timevec(2), '%02d');
    H8_Day = num2str(timevec(3), '%02d');
    ...
    filePath = ['J:\H8Data\Matched_P1\MatchedVFM_CloudProperty_', Time_Current, '.mat'];
    % 检查文件是否存在
