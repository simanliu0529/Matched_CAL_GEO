...
dirCAL='/data/yangyin/liusiman/CALIPSO_Data/'; %加载CALIPSO VFM数据
...
for fileH8_i = 1:fileNum_H8
    FY4A_Cloud_temp = importdata(fileList_H8{fileH8_i,1});%把某波段某月的mat文件读出来
    FY4A_Year = fileList_H8{fileH8_i,3}(1:4);
    FY4A_Month = fileList_H8{fileH8_i,3}(5:6);
    FY4A_Day = fileList_H8{fileH8_i,3}(7:8);
    % 输入的云掩膜应该是完成视差校正的结果
    FY4A_Lon = FY4A_Cloud_temp.Lon'; 
    FY4A_Lat = FY4A_Cloud_temp.Lat';
    FY4A_Cloud_Time = FY4A_Cloud_temp.Time;
    clear FY4A_Cloud_temp
    % 将数据还原为每天24小时观测，无观测时为NaN
    [~,FY4A_Time_All] = fun_H8Data2AllDay(FY4A_Cloud_Time);    
    ...
    for file_i = 1:fileNum_CAL
        ...
        if ~strcmp(Year_temp,FY4A_Year) || ~strcmp(Month_temp,FY4A_Month) || ~strcmp(Day_temp,FY4A_Day)
            continue;
        end       
        ...
        % [VFM_Lat_new,VFM_Lon_new] = Fun_ParallaxCorrection(VFM_Lat, VFM_Lon, VFM_CloudTop(:,4), 2);
        ...
        % 加载 FY4A 经纬度和 CALIPSO 经纬度
        FY4A_Lon_vec = FY4A_Lon(:);
        FY4A_Lat_vec = FY4A_Lat(:);
        FY4A_points = [FY4A_Lon_vec, FY4A_Lat_vec];
        VFM_points = [VFM_Lon_new, VFM_Lat_new];
        % 构建 KD-Tree 并找到最近邻点
        tree = KDTreeSearcher(FY4A_points);
        [Index, Dist] = knnsearch(tree, VFM_points);
        % 获取行列索引
        [Row, Col] = ind2sub(size(FY4A_Lon), Index);
        % [Col,Row] = ind2sub(size(FY4A_Lon), Index);
        % 时间匹配
        FY4A_Time2 = repmat(FY4A_Time_All',length(VFM_time),1);
        VFM_time2 = repmat(VFM_time,1,length(FY4A_Time_All));
        diff_Time = abs(FY4A_Time2-VFM_time2);%某一行为某一个VFM时间和所有H8时间的差。
        [Min_DiffTime,Index_Time] = min(diff_Time,[],2);%对每一个VFM时间，和它最接近的H8时间的index。
        Index_Time(Min_DiffTime>0.5/24) = NaN; % 30min为限制
        % Index_Time(Min_DiffTime>1/288) = NaN; % 5min为限制
        if sum(isnan(Index_Time)) == length(Index_Time); continue; end
        Index_Time = single(Index_Time);

        Index_All = [Row, Col,Index_Time]; %每个VFM点，H8的经纬度和时间索引。
        ...
        if file_i == fileNum_CAL %假如到最后一个文件了就存储
            ...
            save(['/data/yangyin/liusiman/FYData/FY_4A/Matched_P1_uncorrected_all/MatchedVFM_CloudProperty_',Time_Current,'.mat'],'Data_Result','-v7.3');
            Data_Result=[];
