...
%             将一个5515个元素vfm数组，提取其使用者指定的感兴趣位
%             这里取其1-3位，输出值为0-7二进制5515个元素
...
% ------------------------------------------------------------------------------
 ...
 switch lower(type)
   case {'type','all'}
       ...
       ClassText = struct('FieldDescription',{'Vertical Feature Mask'},...   %此处Feature Type修改为Vertical Feature Mask
   ...
   case{'cloud'}
       ...
       'ByteTxt',{{'     NA','Overcast,thin','Overcast,thick','   Tran.StraCu','   Low Broken',...
       '   Altocumulus','   Altostratus','   Cirrus','Deep Convection'}});     
   %由于字母容纳不下，Low overcast,thin和Low,overcast,thick变为Overcast,thin和Overcast,thick
   case{'psc'}
       a = bitshift(vfm_row,-9); 
       temp = bitand(umask3,a);
       type2 = bitand(umask3,vfm_row);
